...
def get_chain_symmetries(cropped, max_n_symmetries=100):
    ...
    crop_to_all_atom_map = []
    # Optimized: Precompute mapping for O(1) lookups instead of O(N) .index() in the loop
    chain_asym_to_idx = {asym_id: i for i, asym_id in enumerate(chain_asym_id)}
    for token in cropped.tokens:
        chain_idx = chain_asym_to_idx[token["asym_id"]]
        start = (
...
def get_ligand_symmetries(cropped, symmetries, return_physical_metrics=False):
    ...
    for mol_name, start_mol, mol_id, mol_atom_names in index_mols:
        if not mol_name in symmetries:
            ...
            # Optimized: Precompute reverse mapping for O(1) lookups instead of O(N) .index()
            name_to_ccd_idx = {name: i for i, name in enumerate(mol_atom_names_ccd)}
            ccd_to_valid_ids = {
                name_to_ccd_idx[name]: i
                for i, name in enumerate(mol_atom_names)
                if name in name_to_ccd_idx
            }
