...
## 2024-06-16 - [List Membership checks]
**Learning:** Using `[seqs_unique.append(x) for x in seqs if x not in seqs_unique]` causes O(N^2) list membership checks.
**Action:** Precompute a dictionary mapping or use `list(dict.fromkeys())` for deduplication to maintain O(1) lookups or O(N) processing.
