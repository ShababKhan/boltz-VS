...
## 2024-05-18 - Generator Expression for Short-Circuiting in all()
**Learning:** Using `all([...])` with a list comprehension inside forces the entire list to be evaluated and allocated in memory before `all()` runs, defeating its ability to short-circuit upon finding a False value.
**Action:** In performance-sensitive contexts, replace list comprehensions inside `all()` or `any()` calls with generator expressions (`all(...)`). This avoids O(N) memory allocation and enables short-circuiting, providing substantial performance gains.
