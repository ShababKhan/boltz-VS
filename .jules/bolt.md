...
## 2024-11-25 - O(N) MSA sequence deduplication
**Learning:** O(N²) list membership checks (`item in list`) and `.index()` lookups for deduplicating multiple sequence alignments (MSAs) cause significant CPU/time bottlenecks when processing large datasets.
**Action:** Use O(N) dictionary-based hash maps or insertion-ordered list deduplication like `list(dict.fromkeys(seqs))` and a mapping dictionary for index lookups (`{seq: i for i, seq in enumerate(seqs_unique)}`) to prevent performance bottlenecks.
